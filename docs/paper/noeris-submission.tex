...
principle. A v2 cold-shape replay removes curated starters and holds out the
largest measured buckets for rmsnorm, softmax, layernorm, and cross-entropy
(\texttt{docs/results/cold-shape-cross-run-ablation-v2.json}). Database-seeded
transfer reaches 90\% of oracle faster when successful, but does not improve
final regret or success rate over stateless random; the best condition is
cost-model ranking (0.997 mean final normalized throughput, 0.29\% mean regret).
We therefore treat the persistent database as replayable training data and
selector context, not standalone evidence of compounding gains from historical
winners.
...
The database is validated as replayable training data for ranking models, but
standalone database-seeded compounding remains unproven.
